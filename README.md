...
**Live demo:** https://hajrazakir.github.io/Taskora/
...
- **Vercel** — `vercel.json`
- **GitHub Pages** — deployed via the included GitHub Actions workflow
  (`.github/workflows/deploy.yml`), which builds the app and publishes the
  `dist/` folder. Deep-link refreshes (e.g. reloading `/dashboard` directly)
  will still 404 on GitHub Pages since it doesn't support rewrites — add a
  `404.html` that redirects to `index.html` if that matters for your use case.
